...
bool StressUpgradeWorker::transferData(const Firmware &firmware, int cycleIndex, int startProgress, QString *errorMessage)
{
    ...
    while (offset < firmware.data.size() && !shouldStop()) {
        ...
        header.append(static_cast<char>(crc8(chunk, chunkSize)));
...
quint8 StressUpgradeWorker::crc8(const char *data, int length) const
{
    ...
    for (int i = 0; i < length; ++i) {
        crc = table[crc ^ static_cast<quint8>(data[i])];
    }
    return crc;
}
